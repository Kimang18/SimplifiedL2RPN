...
if __name__=="__main__":
    ...
    for i in range(6):
        ...
        s = env.reset()
        obs_shape = s.shape
        ag = DDQN(obs_shape, env.amt_lines)
        ag.load_weights("Data/ddqn_weight")
        ag.epsilon = 0.01
        ...
        while i_ep < EPISODES:
            ...
            while True: #for h in range(200):
                ...
                if done:
                    print("Epi: \t{}, Ts: \t{}, total rewards: \t{}, epsilon: \t{}, curr_ts: \t{}".format(i_ep, h, total_rewards, ag.epsilon, current_ts))
            ...
            #if i_ep % 500 == 0:
            #    ag.save_weights("Data/ddqn_weight")
        df[i] = avg_returns
    df.to_csv("Data/multi_run_ddqn.csv", sep=";", index=False)

    """
    ag.save_weights("Data/ddqn_weight")
    np.savetxt("Data/average_returns_ddqn.txt", avg_returns, fmt='%.3f')
    np.savetxt("Data/average_timesteps_ddqn.txt", avg_timesteps, fmt='%.3f')
